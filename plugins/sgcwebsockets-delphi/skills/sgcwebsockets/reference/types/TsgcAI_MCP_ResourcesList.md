...
```pascal
procedure AddResource(const aResource: TsgcAI_MCP_Resource);
function RemoveResource(const aName: string): Boolean;
function GetResource(const aName: string): TsgcAI_MCP_Resource;
