...
Add `IdSimpleServer` to your `uses` clause. Property and event types that link below are documented under `reference/types/`.

## Properties

| Delphi | Type | C++Builder |
| --- | --- | --- |
| `AcceptWait: Integer` | `Integer` | `__property int AcceptWait;` |
...
procedure EndListen;
procedure Listen(ATimeout: Integer = IdTimeoutDefault);
...
void __fastcall BeginListen();
void __fastcall CreateBinding();
void __fastcall EndListen();
void __fastcall Listen(int ATimeout = IdTimeoutDefault);
