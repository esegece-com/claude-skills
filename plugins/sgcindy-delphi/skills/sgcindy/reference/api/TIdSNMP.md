...
Add `IdSNMP` to your `uses` clause. Property and event types that link below are documented under `reference/types/`.
...
| `TrapPort: TIdPort` | `TIdPort` | `__property TIdPort * TrapPort;` |
| `Community: string` | `string` | `__property UnicodeString Community;` |
...
function QuickSendTrap(const DestHost, Enterprise, DestCommunity: string; DestPort: TIdPort; Generic, Specific: integer; MIBValues: TIdMIBValueList ): Boolean;
function QuickReceiveTrap(var SrcHost, Enterprise, SrcCommunity: string; var SrcPort: TIdPort; var Generic, Specific, Seconds: integer; MIBValues: TIdMIBValueList ): Boolean;
...
bool __fastcall QuickSend(const UnicodeString Mib, const UnicodeString DestCommunity, const UnicodeString DestHost, UnicodeString &Value);
bool __fastcall QuickSendTrap(const UnicodeString DestHost, const UnicodeString Enterprise, const UnicodeString DestCommunity, TIdPort * DestPort, int Generic, int Specific, TIdMIBValueList * MIBValues);
bool __fastcall QuickReceiveTrap(UnicodeString &SrcHost, UnicodeString &Enterprise, UnicodeString &SrcCommunity, TIdPort * &SrcPort, int &Generic, int &Specific, int &Seconds, TIdMIBValueList * MIBValues);
