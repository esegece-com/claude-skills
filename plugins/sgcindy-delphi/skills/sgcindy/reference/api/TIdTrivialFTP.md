...
Add `IdTrivialFTP` to your `uses` clause. Property and event types that link below are documented under `reference/types/`.

## Properties

| Delphi | Type | C++Builder |
| --- | --- | --- |
| `TransferMode: TIdTFTPMode` | [TIdTFTPMode](../types/TIdTFTPMode.md) | `__property TIdTFTPMode * TransferMode;` |
| `RequestedBlockSize: Integer` | `Integer` | `__property int RequestedBlockSize;` |
...
```cpp
void __fastcall Get(const UnicodeString ServerFile, TStream * DestinationStream);
void __fastcall Put(TStream * SourceStream, const UnicodeString ServerFile);
