...
Add `IdWhois` to your `uses` clause. Property and event types that link below are documented under `reference/types/`.
...
```cpp
UnicodeString __fastcall WhoIs(const UnicodeString ADomain);
