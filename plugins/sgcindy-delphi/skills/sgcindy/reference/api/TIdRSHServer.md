...
Add `IdRSHServer` to your `uses` clause. Property and event types that link below are documented under `reference/types/`.
...
| `ForcePortsInRange: Boolean` | `Boolean` | `__property bool ForcePortsInRange;` |
...
| Delphi | Type | C++Builder |
| --- | --- | --- |
| `OnCommand: TIdRSHCommandEvent` | [TIdRSHCommandEvent](../types/TIdRSHCommandEvent.md) | `__property TIdRSHCommandEvent OnCommand;` |
...
```cpp
void __fastcall SendError(TIdContext * AThread, TIdTCPClient * AStdErr, UnicodeString AMsg);
void __fastcall SendResults(TIdContext * AThread, TIdTCPClient * AStdErr, UnicodeString AMsg);
