...
Add `IdDICT` to your `uses` clause. Property and event types that link below are documented under `reference/types/`.
...
| `Server: String (read-only)` | `String` | `__property UnicodeString Server /* read-only */;` |
| `TryMIME: Boolean` | `Boolean` | `__property bool TryMIME;` |
| `Client: String` | `String` | `__property UnicodeString Client;` |
| `AuthType: TIdDICTAuthenticationType` | [TIdDICTAuthenticationType](../types/TIdDICTAuthenticationType.md) | `__property TIdDICTAuthenticationType * AuthType;` |
...
procedure Define(const AWord, ADBName : String; AResults : TIdDefinitions);
procedure Match(const AWord, ADBName, AStrat : String; AResults : TIdMatchList);
...
void __fastcall GetDictInfo(const UnicodeString ADict, TStrings * AResults);
void __fastcall GetSvrInfo(TStrings * AResults);
void __fastcall GetDBList(TIdDBList * ADB);
void __fastcall GetStrategyList(TIdStrategyList * AStrats);
void __fastcall Define(const UnicodeString AWord, const UnicodeString ADBName, TIdDefinitions * AResults);
void __fastcall Match(const UnicodeString AWord, const UnicodeString ADBName, const UnicodeString AStrat, TIdMatchList * AResults);
