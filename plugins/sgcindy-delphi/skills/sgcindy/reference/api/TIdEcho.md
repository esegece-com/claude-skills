...
Add `IdEcho` to your `uses` clause. Property and event types that link below are documented under `reference/types/`.

## Properties

| Delphi | Type | C++Builder |
| --- | --- | --- |
| `EchoTime: UInt32 (read-only)` | `UInt32` | `__property UInt32 EchoTime /* read-only */;` |
...
```cpp
UnicodeString __fastcall Echo(const UnicodeString AText);
