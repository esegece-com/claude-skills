...
Add `IdServerIOHandlerStack` to your `uses` clause. Property and event types that link below are documented under `reference/types/`.
...
```pascal
function MakeClientIOHandler(ATheThread:TIdYarn ): TIdIOHandler;
...
```cpp
TIdIOHandler * __fastcall MakeClientIOHandler(TIdYarn * ATheThread);
